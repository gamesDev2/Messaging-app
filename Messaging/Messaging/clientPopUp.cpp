...
joinServerPopUp::joinServerPopUp(int x, int y, int width, int height, const char(*title)) :
	Fl_Double_Window(x, y, width, height, title),
	m_maincol(0, 0, w(), h()),
	m_username(0, 0, 0, 0, "Username: "),
	m_address(0, 0, 0, 0, "Address: "),
	m_port(0, 0, 0, 0, "Port: "),
	m_bottomRow(0, 0, 0, 0),
	...
	m_accepted(false)
{
	...
	m_maincol.add_resizable(m_address);
...
std::string joinServerPopUp::address()
{
	std::string addressStr = m_address.value();
	return addressStr;
}

const char* joinServerPopUp::userName()
{
	return m_username.value();
}

bool joinServerPopUp::dataSubmitted()
{
	return m_accepted;
}
...
void joinServerPopUp::on_close(Fl_Widget* _widget, void* _userdata)
{
	joinServerPopUp* window = (joinServerPopUp*)_userdata;
	window->m_accepted = false;
	window->hide();
}

void joinServerPopUp::on_accept(Fl_Widget* _widget, void* _userdata)
{
	joinServerPopUp* window = (joinServerPopUp*)_userdata;
...
void joinServerPopUp::on_cancel(Fl_Widget* _widget, void* _userdata)
{
	joinServerPopUp* window = (joinServerPopUp*)_userdata;
...
bool joinServerPopUp::checkValid(std::string _port) 
{
